...
function createDoc(name, doc) {
	...
	var input = {
		    ...
		    body: { 
	            contentType : 'application/json',         
	            content : doc //the doc is being passed as STRING representation of a JSON
	        }
		};
...
function updateDoc(name, docID, updatedDoc) {
	var input = {
		    method : 'put',
		    returnedContentType : 'json',
		    path : name + '/' + docID,
...
function getView(name, designDocName, viewName, key) {
	
	var path = name + '/_design/' + designDocName + '/_view/' + viewName + '?key=%22Itayzr%22';
